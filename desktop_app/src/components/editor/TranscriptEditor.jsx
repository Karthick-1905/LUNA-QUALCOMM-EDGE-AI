...
export default function TranscriptEditor({
  ...
  const filteredTranscription = transcription.filter(segment => {
    ...
    // Handle both speaker ID (string) and speaker object formats
    const speakerId = typeof segment.speaker === 'string' ? segment.speaker : segment.speaker?.id;
    const matchesSpeaker = selectedSpeaker === 'all' || speakerId === selectedSpeaker;
  ...
  useHotkeys('e', () => {
    ...
    <div className="flex-1 flex bg-zinc-950 h-full">
      ...
      <div className="flex-1 flex flex-col transition-all duration-300">
        ...
        >
          <div className="p-4 space-y-3">            {filteredTranscription.map((segment, index) => {
              // Handle both speaker ID and speaker object formats
              const speaker = typeof segment.speaker === 'string' 
                ? getSpeaker(segment.speaker)
                : segment.speaker;
      ...
      {showEditingTools && (
        <div className="w-80 border-l border-zinc-800 bg-zinc-950 flex flex-col flex-shrink-0">
          <div className="p-4 border-b border-zinc-800">
            ...
            </div>            {activeEditingSegment && (
              <div className="text-sm text-zinc-400 mt-1">
                Editing: {(typeof activeEditingSegment.speaker === 'string' 
                  ? getSpeaker(activeEditingSegment.speaker)
                  : activeEditingSegment.speaker)?.name || 'Unknown'} • {formatTime(activeEditingSegment.start)}
              </div>
            )}
          </div>
